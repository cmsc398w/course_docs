...
- *-n*: Tell me exactly which line the comment is on!
...
*Tab completion* saves time and prevents typos. If you aren't mashing the Tab key, you're working too hard.
...
Note: =command &> file= works in bash/zsh; the portable form is =command > file 2>&1=.
...
Find all lines with "Error" in =app.log= and count them.
...
#+ATTR_BEAMER: :overlay +-
1. *Redirection Error:* =>= sends output to a *file* named =wc=. Use =|= to send to a *program*.
2. *Ambiguous Output:* =wc= shows lines, words, AND characters. Use =wc -l= for just lines.
...
| =-0=       | Read NUL-delimited input (safe)       |
| =-P N=     | Run N processes in parallel (Fast!)   |
...
|---------+-------------+----------------+-------------------|
| Tools   | =grep=, =sed=  | =grep -E=, =awk=  | =grep -P=, Python  |
| =+=, =?=  | Need =\=    | Raw: =+=, =?=  | Raw: =+=, =?=     |
| =()=, ={}= | Need =\=    | Raw: =()=, ={}= | Raw: =()=, ={}=    |

#+ATTR_BEAMER: :overlay +-
- *Note:* macOS BSD =grep= usually lacks =-P=. Prefer =grep -E= or install GNU =grep= (often =ggrep=).

*** Example: Matching "1 or more"
:PROPERTIES:
:BEAMER_ENV: exampleblock
:END:
...
| *Quantify* | =*= / =+= | Zero+ / One+ repetitions              |
| *Groups*   | =[]= / =()= | Sets / Capture groups                 |
...
#+ATTR_BEAMER: :overlay +-
- =^apple= matches "apple pie" but not "red apple"
- =dog$= matches "hot dog" but not "dogwood"
- =ba+na= matches "banana", "baana", but not "bana"
...
=sed= works line-by-line. It never modifies the original file unless you use the =-i= (in-place) flag.
...
=awk= automatically loops over every line. You define *patterns* and *actions*.
...
2. *Answer:* =grep "$1" "$2"=
- Use =$@= for looping over arguments.
...
- =for f in *= is safer and faster than =for f in $(ls)=.
