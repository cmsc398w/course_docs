...
#+LATEX_HEADER: \usepackage[width=3840]{beamer-reveal}
...
2. THis is also a note about something
3. THis is all I have to say about that thing
...
#+ATTR_BEAMER: :overlay +-
- =^apple= matches: *"apple pie"* only (starts with apple)
- =apple$= matches: *"red apple"* only (ends with apple)
- =^dog$= matches: *"dog"* only (entire line is exactly "dog")
- =dog= matches: *"dog", "hot dog", "dogwood"* (contains dog anywhere)
- =[0-9]= matches: *"error123", "ERROR456"* (contains any digit)
- =^[a-z]= matches: *"apple pie", "red apple", "banana", "dog", "hot dog", "dogwood", "error123"* (starts with lowercase)
...
| Tool | Flavor | Example |
|------+--------+---------|
| =grep= (default) | Basic (BRE) | =grep 'pattern'= |
| =grep -E= | Extended (ERE) | =grep -E 'pattern+'= |
| =sed= (default) | Basic (BRE) | =sed 's/old/new/'= |
| =awk= | Extended (ERE) | =awk '/pattern+/'= |
...
Created by Lee E. McMahon at Bell Labs in 1973-74, =sed= was designed for editing files in batch mode. It uses *Basic Regular Expressions (BRE)* by default, which is why some special characters need escaping.
...
** sed: Search and Replace Examples
:PROPERTIES:
:BEAMER_OPT: fragile
:END:
...
Created in 1977 by *A*ho, *W*einberger, and *K*ernighan at Bell Labs (hence "awk"). It's actually a full programming language! Unlike =sed=, =awk= uses *Extended Regular Expressions (ERE)* by default.
