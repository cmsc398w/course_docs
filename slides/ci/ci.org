...
- Lilli tries to deploy Friday at 5pm. It fails.
- Dhruv now pushes a feature at 5:01pm. It fails.
- Jadon pushes another feature at 5:02pm. It also fails.
...
Each box is a job. The arrows become =needs:= declarations. =lint= has no =needs=. =test-backend= and =test-frontend= both say =needs: [lint]=. =db-migrate= says =needs: [test-backend, test-frontend]=.
...
\begin{tikzpicture}[
    ...
    task/.style={rectangle, draw, fill=blue!15, text width=5.5em, text centered,
                 rounded corners, minimum height=2.5em, thick, font=\small},
    artifact/.style={rectangle, draw, fill=orange!20, text width=5.5em, text centered,
                rounded corners, minimum height=2.5em, thick, font=\small},
    line/.style={draw, -latex', thick}]
    ...
    \node [artifact, below right=1.8cm and -0.5cm of bundle] (deploy) {Deploy};
...
\begin{tikzpicture}[
    ...
    task/.style={rectangle, draw, fill=blue!15, text width=5em, text centered,
                 rounded corners, minimum height=2em, thick, font=\small},
    artifact/.style={rectangle, draw, fill=orange!20, text width=5em, text centered,
                rounded corners, minimum height=2em, thick, font=\small},
    line/.style={draw, -latex', thick}]
    \node [src] (css)    {styles.css};
    ...
    \node [artifact, below right=1.4cm and -0.3cm of bundle] (deploy) {Deploy};
    \path [line] (css) -- (bundle);
    \path [line] (py)  -- (pytest);
    \path [line] (bundle) -- (deploy);
    \path [line] (pytest) -- (deploy);
